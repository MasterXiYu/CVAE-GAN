...
class VAE_GAN(torch.nn.Module):
	def __init__(self):
		...
		self.Encoder_net = nn.Sequential(
			...
			nn.BatchNorm2d(N_FEATURE),
			nn.ReLU(),
			nn.Conv2d(N_FEATURE, LATENT_CODE_NUM, kernel_size=4),
		).cuda()
		...
		self.Decoder_net = nn.Sequential(nn.ConvTranspose2d(LATENT_CODE_NUM, 4 * N_FEATURE, kernel_size=4, bias=False),
										 ...
										 nn.BatchNorm2d(N_FEATURE),
										 nn.LeakyReLU(),
										 nn.ConvTranspose2d(N_FEATURE, N_FEATURE // 2, kernel_size=4, padding=1,
															stride=2, bias=False),
										 nn.BatchNorm2d(N_FEATURE // 2),
										 nn.LeakyReLU(),
										 nn.ConvTranspose2d(N_FEATURE // 2, N_FEATURE // 4, kernel_size=4, padding=1,
															stride=2, bias=False),
										 nn.BatchNorm2d(N_FEATURE // 4),
										 nn.LeakyReLU(),
										 nn.ConvTranspose2d(N_FEATURE // 4, N_CHANNEL, kernel_size=4, padding=1,
															stride=2, bias=False),
										 nn.Sigmoid(),
										 ).cuda()
...
def loss_VAE(fake_img, real_img, mu, logvar, output_fake, output_real):
	batch_size = real_img.size(0)
	labels_real = torch.ones(batch_size).cuda()
	labels_fake = torch.zeros(batch_size).cuda()
	labels_GAN = torch.ones(batch_size).cuda()
	BCE = F.binary_cross_entropy(fake_img, real_img, size_average=False)
...
if __name__ == '__main__':
	...
	for epoch in range(epoch_num):
		for batch_idx, data in enumerate(dataloader):
			...
			optimizer.zero_grad()
			recon_img, mu, logvar, output_fake, output_real = vae_gan.forward(img_real)
			loss = loss_VAE(fake_img=recon_img, real_img=img_real, mu=mu, logvar=logvar, output_fake=output_fake,
